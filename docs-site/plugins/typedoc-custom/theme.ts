...
interface EndpointInventory {
  blocks: Record<string, InventoryBlockEntry>
  flows: Record<string, { blocks: string[] }>
  hooks: Record<string, InventoryBlockEntry>
}

let cachedInventory: EndpointInventory | null = null

/**
 * The endpoint inventory (`endpoint-inventory.json`) is generated by
 * `build/deriveEndpointInventory.ts` ahead of TypeDoc in the `derive` pipeline.
 * Keys are shared across both generators: `Namespace.Component` for blocks and
 * flows, bare hook names for hooks.
 */
function getEndpointInventory(): EndpointInventory {
  if (!cachedInventory) {
...
const ENDPOINT_METHOD_RANK: Record<string, number> = {
  GET: 0,
  POST: 1,
  PUT: 2,
  PATCH: 3,
...
/** Dedupe by method+path, then sort by path (then method) to match endpoint-reference.md. */
function sortEndpoints(endpoints: InventoryEndpoint[]): InventoryEndpoint[] {
  ...
  return endpoints
    ...
    .sort((a, b) => {
      ...
      return (ENDPOINT_METHOD_RANK[a.method] ?? 99) - (ENDPOINT_METHOD_RANK[b.method] ?? 99)
    })
}

/** Endpoints a block or hook calls, by inventory key. */
function endpointsForBlockOrHook(key: string | undefined): InventoryEndpoint[] {
  if (!key) return []
  const inventory = getEndpointInventory()
  const entry = inventory.blocks[key] ?? inventory.hooks[key]
...
/** Markdown `Endpoints` section (heading + table), or `''` when there are none. */
function renderEndpointsSection(endpoints: InventoryEndpoint[], headingLevel: number): string {
...
export class SDKTheme extends MarkdownTheme {
  ...
  static injectFrontmatter(page: MarkdownPageEvent): void {
    // The project index must be .mdx so DocCardList JSX renders correctly.
    if (page.model.kind === ReflectionKind.Project) {
      page.filename = page.filename.replace(/index\.md$/, 'index.mdx')
    }
  ...
  constructor(...args: ConstructorParameters<typeof MarkdownThemeContext>) {
    ...
    this.templates = {
      ...
      reflection: (page: MarkdownPageEvent<DeclarationReflection>) => {
        ...
        const router = this.router as SDKRouter
        const flowGuide = router.flowGuides.get(page.model)
        if (flowGuide) rendered = renderGuidePage(rendered, flowGuide)
        const hookGuide = router.hookGuides.get(page.model)
        if (hookGuide) rendered = renderHookGuidePage(rendered, hookGuide)

        // Append the endpoints this page's flow or hook calls, from the shared
        // endpoint inventory. Flows show the union of their blocks' endpoints.
        const endpointKey = router.endpointKeys.get(page.model)
        const endpoints =
          endpointKey && getEndpointInventory().flows[endpointKey]
            ? endpointsForFlow(endpointKey)
            : isHookPage(page.model)
              ? endpointsForBlockOrHook(page.model.name)
              : []
        const endpointsSection = renderEndpointsSection(endpoints, 2)
        if (endpointsSection) rendered = `${rendered.trimEnd()}\n\n${endpointsSection}\n`

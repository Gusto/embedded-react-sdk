...
import scopedHeadingIds from './plugins/scoped-heading-ids/index.ts'
    ...
    [
      'classic',
      {
        docs: {
          ...
          breadcrumbs: true,
          exclude: ['test-fests/**'],
          beforeDefaultRemarkPlugins: [scopedHeadingIds],
          ...
          // This generator removes the duplicate child item and reorganizes the reference
          // section into groups with HTML dividers between them.
          sidebarItemsGenerator: async ({ defaultSidebarItemsGenerator, item, ...rest }) => {
            const items = await defaultSidebarItemsGenerator({ item, ...rest })

            if (item.dirName !== 'reference') {
              return items.filter(i => !(i.type === 'doc' && i.id === 'reference/index'))
            }

...
async function ensureProcessedPayroll(flowToken: string, companyId: string): Promise<boolean> {
  const today = new Date()
  const rangeStart = new Date(today)
  ...
  console.log(
    `[transition-setup] Processing payroll ${payrollUuid} (${unprocessedPast.start_date} to ${unprocessedPast.end_date})...`,
  )
...
async function createTransitionPeriodViaScheduleChange(
  ...
  if (!hasHistory) {
    console.log(
      '[transition-setup] Cannot create transition period without processed payroll history',
    )
    return null
  }
  ...
  console.log(
    `[transition-setup] Current schedule: frequency="${schedule.frequency}", version="${schedule.version}"`,
  )
  ...
  console.log(
    '[transition-setup] First frequency change did not produce a transition period, retrying with toggle...',
  )
...
test.describe('TransitionFlow', () => {
  ...
  test.beforeAll(async () => {
    ...
    if (!flowToken || !companyId) {
      console.warn('[transition-setup] Missing flowToken or companyId')
      return
    }

    let payScheduleUuid = state.payScheduleUuid
    if (!payScheduleUuid) {
      const schedules = await fetchApi<PaySchedule[]>(
        `/fe_sdk/${flowToken}/v1/companies/${companyId}/pay_schedules`,
      )
      payScheduleUuid = schedules[0]?.uuid
      if (!payScheduleUuid) {
        console.warn('[transition-setup] No pay schedule found for company')
        return
      }
      console.log(`[transition-setup] Found pay schedule: ${payScheduleUuid}`)
    }
    ...
    try {
      ...
      if (existingPeriods.length > 0) {
        console.log(
          `[transition-setup] Reusing existing transition period: ${existingPeriods[0].start_date} to ${existingPeriods[0].end_date}`,
        )
      ...
      const result = await createTransitionPeriodViaScheduleChange(
        flowToken,
        companyId,
        payScheduleUuid,
      )
      if (result) {
        console.log(
          `[transition-setup] Created transition period: ${result.startDate} to ${result.endDate}`,
        )
        transitionState.ready = true
  ...
  test.describe('Deductions and Tax Config Interaction', () => {
    ...
    test('can open and close tax withholding edit modal', async ({ page }) => {
      ...
      await expect(page.getByText('Rate for regular wages and earnings')).toBeVisible({
        timeout: 10000,
      })
      ...
      await expect(page.getByText('Rate for regular wages and earnings')).not.toBeVisible({
        timeout: 5000,
      })
  ...
  test.describe('Creation to Execution', () => {
    ...
    test('creates payroll with blocked deductions and enters execution', async ({
      ...
      await expect(page.getByLabel(/block all deductions and contributions/i)).toBeChecked()
  ...
  test.describe('Full Flow', () => {
    test('completes full flow from creation through payroll receipt', async ({
      ...
      await expect(page.getByRole('heading', { name: /payroll summary/i, level: 1 })).toBeVisible({
        timeout: 120_000,
      })

      await expect(page.getByRole('button', { name: /view payroll receipt/i })).toBeVisible({
        timeout: 10000,
      })
    })
  })

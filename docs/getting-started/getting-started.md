...
function handleProxyRequest(request, authenticatedUser) {
  const allowlistByRole = {
    payroll_admin: () =>
      buildAllowlist({
        flows: ['Employee.OnboardingFlow', 'Payroll.PayrollFlow'],
        variables: { companyId: authenticatedUser.companyId },
      }),
    onboarding_admin: () =>
      buildAllowlist({
        blocks: [
  ...
  const allowlist = (allowlistByRole[authenticatedUser.role] ?? (() => []))()

  if (!allowlist.some(e => e.method === request.method && e.path === request.path)) {
    return 403
  }

  return forwardToGustoAPI(request, authenticatedUser.oauthToken)
}
```

`buildAllowlist` accepts `flows`, `blocks`, and `variables`. Flows include all endpoints for an entire workflow. Blocks target individual components for finer control. The `variables` field replaces `:param` placeholders with actual values from the user's session -- for self-service employees, this locks every path to their specific employee ID, combining endpoint allowlisting and resource ownership validation into one step.

For complete, runnable Express.js examples and detailed guidance, see the [Proxy Security: Partner Guidance](./proxy-security-partner-guidance.md#complete-example-express-proxy-with-sdk-permission-sets).

...
You can supply a function to this callback and respond to events as needed. In the following example we set up an event handler for the [`EmployeeOnboarding.Profile`](../api/Employee/EmployeeOnboarding/blocks.md#profile) component and execute code based on the event type:

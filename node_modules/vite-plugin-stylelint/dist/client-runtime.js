...
const WS_RUNTIME_LOADED_EVENT = "vite:stylelint:runtime-loaded";
const WS_OVERLAY_EVENT = "vite:stylelint:overlay";
const OVERLAY_TAG = "vite-plugin-stylelint-overlay";
const DEFAULT_THEME = {
	"--vite-plugin-stylelint-bg": "#1e1e1e",
	"--vite-plugin-stylelint-panel-bg": "#252525ee",
	"--vite-plugin-stylelint-error": "#f48771",
	"--vite-plugin-stylelint-warning": "#cca700",
	"--vite-plugin-stylelint-text": "#d4d4d4",
	"--vite-plugin-stylelint-font-mono": "ui-monospace, SFMono-Regular, 'SF Mono', Menlo, Consolas, monospace",
	"--vite-plugin-stylelint-radius": "6px"
};
...
var VitePluginStylelintOverlay = class extends HTMLElement {
	constructor() {
		super();
	...
	renderShell() {
      ...
      <style>
        :host {
          ...
          ${pos.host}
          z-index: ${zIndex};
          font-family: var(--vite-plugin-stylelint-font-mono);
          color: var(--vite-plugin-stylelint-text);
        }
        .badge {
          display: inline-flex;
          align-items: center;
          ...
          margin: 16px;
          padding: 8px 14px;
          background: var(--vite-plugin-stylelint-bg);
          color: var(--vite-plugin-stylelint-text);
          border-radius: var(--vite-plugin-stylelint-radius);
          cursor: pointer;
          user-select: none;
          font-size: 13px;
          line-height: 1;
        ...
        .badge .dot {
          ...
          height: 8px;
          border-radius: 50%;
          background: var(--vite-plugin-stylelint-error);
        }
        .badge .dot.warn { background: var(--vite-plugin-stylelint-warning); }
        ...
        .panel {
          ...
          overflow: auto;
          background: var(--vite-plugin-stylelint-panel-bg);
          border-radius: var(--vite-plugin-stylelint-radius);
        ...
        .msg .sev.error { color: var(--vite-plugin-stylelint-error); }
        .msg .sev.warning { color: var(--vite-plugin-stylelint-warning); }
        .msg .loc {
          flex: 0 0 auto;
          opacity: 0.6;
        }
      ...
      <div class="badge" part="badge">
        <span class="dot"></span>
        <span class="label">Stylelint</span>
      </div>
	...
	renderBody() {
		...
		if (errors > 0) {
			dot.className = "dot";
			label.textContent = `Stylelint ${errors} error${errors > 1 ? "s" : ""}${warnings > 0 ? ` / ${warnings} warning${warnings > 1 ? "s" : ""}` : ""}`;
		} else if (warnings > 0) {
			dot.className = "dot warn";
			label.textContent = `Stylelint ${warnings} warning${warnings > 1 ? "s" : ""}`;
		} else {
			dot.className = "dot ok";
			label.textContent = "Stylelint";
		}
...
function inject({ overlayConfig }) {
	...
	if (!customElements.get(OVERLAY_TAG)) customElements.define(OVERLAY_TAG, VitePluginStylelintOverlay);

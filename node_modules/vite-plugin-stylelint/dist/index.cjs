const require_linter = require("./linter-CgSKosc9.cjs");
...
vite = require_linter.__toESM(vite, 1);
...
import { inject } from "${require_linter.RUNTIME_CLIENT_RUNTIME_PATH}";
inject({ overlayConfig: ${JSON.stringify(overlayConfig)} });
`;
//#endregion
//#region src/utils.ts
const getOptions = ({ test, dev, build, cache, include, exclude, stylelintPath, formatter, lintInWorker, lintOnStart, lintDirtyOnly, emitError, emitErrorAsWarning, emitWarning, emitWarningAsError, customOverlay, ...stylelintOptions }) => ({
	test: test ?? false,
	dev: dev ?? true,
	build: build ?? false,
	cache: cache ?? true,
...
//#endregion
//#region src/index.ts
const debug$1 = (0, debug.default)(require_linter.PLUGIN_NAME);
...
const RUNTIME_DIST_PATH = (0, node_path.resolve)(__dirname$1, "client-runtime.js");
...
const getRuntimeCode = () => {
	...
	if ((0, node_fs.existsSync)(RUNTIME_DIST_PATH)) {
		cachedRuntimeCode = `${(0, node_fs.readFileSync)(RUNTIME_DIST_PATH, "utf-8")}\n`;
		return cachedRuntimeCode;
	}
	throw new Error(`[${require_linter.PLUGIN_NAME}] custom overlay runtime bundle not found at ${RUNTIME_DIST_PATH}. Run the build before using customOverlay.`);
};
...
function StylelintPlugin(userOptions = {}) {
	...
	const pushOverlay = (server, payload) => {
		...
		if (degraded) return;
		if (runtimeLoaded) {
			server.ws.send(require_linter.WS_OVERLAY_EVENT, payload);
			return;
		}
		if (!degradeTimer) degradeTimer = setTimeout(() => {
			...
			if (!warnedDegraded) {
				warnedDegraded = true;
				logger?.warn(`[${require_linter.PLUGIN_NAME}] custom overlay runtime did not load within the grace window. This environment (e.g. mini-program, SSR without index.html) is not supported. Falling back to terminal-only output. Set customOverlay: false to silence this.`);
			}
	...
	const plugin = {
		name: require_linter.PLUGIN_NAME,
		...
		configureServer(server) {
			devServerRef = server;
			if (!customOverlayEnabled) return;
			server.ws.on(require_linter.WS_RUNTIME_LOADED_EVENT, () => {
				runtimeLoaded = true;
				if (degradeTimer) {
					clearTimeout(degradeTimer);
					degradeTimer = void 0;
				}
				if (pendingPayload !== void 0) server.ws.send(require_linter.WS_OVERLAY_EVENT, pendingPayload);
			});
		},
		async buildStart() {
			...
			if (options.lintInWorker) {
				...
				worker = new node_worker_threads.Worker((0, node_path.resolve)(__dirname$1, `worker${ext}`), {
					workerData: { options },
					env: getWorkerEnv()
				});
				if (customOverlayEnabled) worker.on("message", (msg) => {
					if (msg?.type === "overlay-payload" && devServerRef) pushOverlay(devServerRef, msg.payload);
				});
				return;
			}
			debug$1("Initial Stylelint");
			const useCustomOverlayInServe = customOverlayEnabled && command === "serve";
			linter = require_linter.createLinter(options, {
		...
		resolveId(id) {
			if (id === "/@vite-plugin-stylelint-runtime" || id === "/@vite-plugin-stylelint-runtime-entry") return wrapVirtualPrefix(id);
		},
		load(id) {
			if (id === wrapVirtualPrefix("/@vite-plugin-stylelint-runtime")) return getRuntimeCode();
			if (id === wrapVirtualPrefix("/@vite-plugin-stylelint-runtime-entry")) return composePreambleCode({ overlayConfig: options.customOverlay });
		},
		transformIndexHtml() {
		...
		async transform(_, id) {
			...
			return await linter.lint(ids, this);
		},
		async buildEnd() {

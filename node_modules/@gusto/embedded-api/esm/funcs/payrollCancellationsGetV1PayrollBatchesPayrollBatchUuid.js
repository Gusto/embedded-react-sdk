...
import { GetV1PayrollBatchesPayrollBatchUuidRequest$outboundSchema, GetV1PayrollBatchesPayrollBatchUuidResponse$inboundSchema, } from "../models/operations/getv1payrollbatchespayrollbatchuuid.js";
 ...
 */
export function payrollCancellationsGetV1PayrollBatchesPayrollBatchUuid(client, security, request, options) {
    return new APIPromise($do(client, security, request, options));
}
async function $do(client, security, request, options) {
    const parsed = safeParse(request, (value) => GetV1PayrollBatchesPayrollBatchUuidRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const body = null;
    const pathParams = {
        payroll_batch_uuid: encodeSimple("payroll_batch_uuid", payload.payroll_batch_uuid, { explode: false, charEncoding: "percent" }),
    };
    const path = pathToFunc("/v1/payroll_batches/{payroll_batch_uuid}")(pathParams);
    ...
    const [result] = await M.match(M.json(200, GetV1PayrollBatchesPayrollBatchUuidResponse$inboundSchema, {
        key: "Payroll-Batch-Results",
    }), M.jsonErr([404, 410], NotFoundErrorObject$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=payrollCancellationsGetV1PayrollBatchesPayrollBatchUuid.js.map
...
So: reorder *sections* via `groupOrder` / the group-name arrays; reorder *items within a
section* via `sort` or names; move an item between sections by changing its `@group` tag.
...
If you change routing *logic* (not just config), run the docs-site tests

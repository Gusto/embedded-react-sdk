...
[`docs-shared.md`](../docs-shared.md). Deep reference internals are in `REFERENCE.md` in
...
the release build) does not. If you touched reference *structure*, also run
...
- **Sidebar grouping** — put a hand-authored page and a generated reference page in the
  same category. ⚠️ The generated page is *already* in the `{ type: 'autogenerated' }`
...
- **Page-level prose injection** — fold hand-authored prose *into* a generated hub/flow
  page via a `GUIDE.md` slot, so narrative and generated API tables read as one page
  (mechanism in `REFERENCE.md`).

If you only need the pages *near* the reference (not merged), sidebar grouping is simpler

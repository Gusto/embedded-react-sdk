...
interface UseClientPaginationOptions<TItem> {
  /**
   * Optional substring/fuzzy/etc. predicate run against the deferred search
   * value. The hook owns search state and resets to page 1 whenever the
   * value changes. Omit to skip the search input entirely.
   */
...
export function useClientPagination<TItem>(
  ...
  const searchFilteredItems = useMemo<TItem[]>(() => {
    if (!deferredSearchValue || !searchPredicate) return allItems
    return allItems.filter(item => searchPredicate(item, deferredSearchValue))

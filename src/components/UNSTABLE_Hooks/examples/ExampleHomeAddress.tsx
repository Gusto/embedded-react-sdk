...
function ExampleHomeAddressRoot({ employeeId, onEvent }: ExampleHomeAddressProps) {
  ...
  const { onSubmit, isPending, errors } = homeAddressForm
...
export interface HomeAddressFormFieldsProps {
  form: HomeAddressFormReady
}

export function HomeAddressFormFields({ form }: HomeAddressFormFieldsProps) {
  const { t } = useTranslation(I18N_NS)

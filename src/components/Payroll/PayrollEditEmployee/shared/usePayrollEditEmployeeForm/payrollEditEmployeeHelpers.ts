...
function buildWeekMap(
  ...
  breakdowns:
    Array<{ startDate?: Date; endDate?: Date; hours?: string; amount?: string }> | undefined,
  isSplit: boolean,
  ...
  workweeks.forEach((workweek, index) => {
    ...
    const breakdown = breakdowns?.find(
      entry =>
        entry.startDate &&
        entry.endDate &&
        formatWireDateToStringDate(entry.startDate) === workweek.startDate &&
        formatWireDateToStringDate(entry.endDate) === workweek.endDate,
    )

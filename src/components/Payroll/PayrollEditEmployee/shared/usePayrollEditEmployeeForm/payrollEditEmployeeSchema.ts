...
// non-negative number.
const nonNegativeAmount = z.string().refine(
  value => {
...
export function normalizeWorkweeks(
  ...
  if (normalized.length === 0 && payPeriod?.startDate && payPeriod.endDate) {
...
export function derivePayrollEditEmployeeDefaults(
  ...
  for (const compensation of employeeCompensation?.hourlyCompensations ?? []) {
    ...
    hours[compensation.jobUuid] ??= {}
    hours[compensation.jobUuid]![compensation.name] = buildWeekMap(
      workweeks,
      compensation.hours,
  ...
  for (const compensation of employeeCompensation?.fixedCompensations ?? []) {
    ...
    if (overtimeEarningNames.has(compensation.name)) {
      additionalEarnings[compensation.jobUuid] ??= {}
      additionalEarnings[compensation.jobUuid]![compensation.name] = buildWeekMap(
        workweeks,
        compensation.amount,
        compensation.breakdowns,
        isSplit,
      ...
      other[compensation.jobUuid] ??= {}
      other[compensation.jobUuid]![compensation.name] = formatAmountInput(compensation.amount)
    }
  }
...
function sumWeekValues(weekMap: Record<string, string>, workweeks: NormalizedWorkweek[]): string {
  const sum = workweeks.reduce(
    (total, workweek) => total + (parseFloat(weekMap[workweek.startDate] ?? '0') || 0),
    0,
  )
  return String(sum)
}
...
export function buildPayrollUpdateEmployeeCompensation(
  ...
  const originalHours = originalTotals(employeeCompensation?.hourlyCompensations, comp => comp.hours)
    ...
    return Object.entries(names).map(([name, weekMap]) =>
      jobActive
        ? {
            jobUuid,
            name,
            hours: sumWeekValues(weekMap, workweeks),
  ...
  const breakdownEarnings = Object.entries(formData.additionalEarnings).flatMap(([jobUuid, names]) => {
    const jobActive = isJobActive(names)
    return Object.entries(names).map(([name, weekMap]) => {
      if (jobActive) {
        return {
          jobUuid,
          name,
          amount: sumWeekValues(weekMap, workweeks),
          breakdowns: workweeks.map(workweek => ({
            startDate: new RFCDate(workweek.startDate),
            endDate: new RFCDate(workweek.endDate),

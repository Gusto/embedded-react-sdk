...
describe('StateTaxesEditForm', () => {
  ...
  it('renders the state heading and form fields', async () => {
    renderWithProviders(<StateTaxesEditForm employeeId="employee-1" onEvent={mockOnEvent} />)
  ...
  it('emits the scoped CANCELLED event when the Cancel button is clicked without submitting', async () => {
  ...
  it('emits the scoped UPDATED event with the latest state taxes list on save', async () => {
    const user = userEvent.setup()
    renderWithProviders(<StateTaxesEditForm employeeId="employee-1" onEvent={mockOnEvent} />)

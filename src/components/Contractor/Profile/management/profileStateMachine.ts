...
export const profileStateMachine = {
  card: state<MachineTransition>(
    ...
    transition(componentEvents.CONTRACTOR_MANAGEMENT_PROFILE_ALERT_DISMISSED, 'card', returnToCard),
  ),
  editProfile: state<MachineTransition>(
    ...
    transition(componentEvents.CONTRACTOR_MANAGEMENT_PROFILE_EDIT_CANCELLED, 'card', returnToCard),
  ),
}

...
export function useContractorProfile({
  ...
  const formDefaultValues = {
    ...
    ...(existingContractor && {
      ...
      startDate: existingContractor.startDate ? new Date(existingContractor.startDate) : new Date(),
  ...
  const onSubmit: SubmitHandler<ContractorProfileFormData> = async data => {
    await baseSubmitHandler(data, async payload => {
      if (contractorId && existingContractor) {
        ...
        const apiPayload = transformFormDataToUpdatePayload(
          payload,
          existingContractor.version as string,
        )

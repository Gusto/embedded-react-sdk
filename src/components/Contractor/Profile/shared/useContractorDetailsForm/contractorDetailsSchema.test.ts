 ...
 *
 * Applicability gating is internal to the schema: it derives the inapplicable
 * fields from the values under validation, so each scenario just parses the
 * values it would render. The baseline mirrors the API (ssn/ein optional;
 * create-required fields optional on update); `optionalFieldsToRequire` is
 * exercised where a consumer (e.g. ContractorProfile) tightens beyond it.
 */
...
const parse = (values: Record<string, unknown>, options: SchemaOptions = {}) => {
  const [schema] = createContractorDetailsSchema(options)

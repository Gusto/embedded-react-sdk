...
import { useContractorCompensationSummary } from './useContractorCompensationSummary'
import { GustoTestProvider } from '@/test/GustoTestApiProvider'
import { server } from '@/test/mocks/server'
import { handleGetContractor } from '@/test/mocks/apis/contractors'
import { setupApiTestMocks } from '@/test/mocks/apiServer'

describe('useContractorCompensationSummary', () => {
  beforeEach(() => {
    setupApiTestMocks()
  })
  ...
  it('starts in the loading branch and resolves into the ready branch with the contractor', async () => {
    const { result } = renderHook(
      () => useContractorCompensationSummary({ contractorId: 'contractor-123' }),
    ...
    expect(result.current.data.contractor).toMatchObject({ wageType: 'Hourly' })

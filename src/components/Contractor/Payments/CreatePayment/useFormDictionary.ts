...
/**
 * Resolves `SetPaymentAmounts`'s text against `CreatePayment`'s existing
 * `Contractor.Payments.CreatePayment` namespace, preserving the original key
 * names so partner overrides on that namespace keep working unchanged.
 *
 * @internal
 */
export function useCreatePaymentAmountsDictionary(): SetPaymentAmountsDictionary {
  ...
  return useMemo<SetPaymentAmountsDictionary>(
    () => ({
      en: {
        ...
        alerts: {
          // Re-exported as a template: {{contractorName}} is passed back as its own
          // placeholder so SetPaymentAmounts can interpolate the real name later.

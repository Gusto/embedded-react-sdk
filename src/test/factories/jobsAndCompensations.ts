...
function assertValidCompensation(fixture: CompensationFixture): CompensationFixture {
  ...
  if (!result.ok) {
    throw new Error(
      `buildCompensation produced a fixture that does not match @gusto/embedded-api Compensation schema: ${result.error.message}`,
    )
  }
  return fixture
}
...
function assertValidJob(fixture: JobFixture): JobFixture {
  ...
  if (!result.ok) {
    throw new Error(
      `buildJob produced a fixture that does not match @gusto/embedded-api Job schema: ${result.error.message}`,
    )
  }
  return fixture
}

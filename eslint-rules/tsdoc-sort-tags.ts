 ...
 *   2. @remarks
 *   3. @events — the component's onEvent surface, its own block
 *   4. @components — the flow's composed components/hooks, its own block
 *   5. All other tags (params, returns, throws, deprecated, release tags, etc.)
 *      sorted by TAG_ORDER; tags not in the list sort to the end (stable)
 *   6. @example — each one is its own group
 *
...
function buildGroups(chunks: string[][]): string[][] {
  ...
  for (const chunk of tagChunks) {
    ...
    else if (tag === '@events') eventsChunks.push(chunk)
  ...
  return [
    ...
    eventsChunks.flatMap(c => stripTrailingBlanks(c)),
...
export default ESLintUtils.RuleCreator.withoutDocs({
  meta: {
    ...
    messages: {
      incorrectGrouping:
        'TSDoc tags are not correctly grouped. Expected: description, @remarks, @events, @components, other tags, then each @example as its own group, each separated by a blank line.',
    },
  },

 ...
 *
 * It produces the maps consumed by the SDK Dev App's component registry and settings panel:
 ...
 *   - UNSTABLE_FEATURES_VALUES: one entry per key on the UnstableFeatures type, so the
 *     settings panel's feature-flag checkboxes stay in sync without editing sdk-app
 *
...
const ENTITY_ID_PATTERN = /^(company|employee|contractor|payroll|request|paymentGroup)Id$/

const UNSTABLE_FEATURES_PATH = 'src/contexts/UnstableFeaturesProvider/useUnstableFeatures.ts'
...
function extractUnstableFeatureKeys(project: Project): string[] {
  const sourceFile = project.addSourceFileAtPath(resolve(ROOT, UNSTABLE_FEATURES_PATH))
  const iface = sourceFile.getInterfaceOrThrow('UnstableFeatures')
  return getPropertyNames(iface.getType(), false).sort()
}
...
async function generate() {
  ...
  const unstableFeaturesProject = new Project({
    tsConfigFilePath: resolve(ROOT, 'tsconfig.json'),
    skipAddingFilesFromTsConfig: true,
  })
  const unstableFeatureKeys = extractUnstableFeatureKeys(unstableFeaturesProject)
  ...
  output += `import type { UnstableFeatures } from '@/contexts/UnstableFeaturesProvider/useUnstableFeatures'\n\n`
  ...
  output += `export const UNSTABLE_FEATURES_VALUES: Required<UnstableFeatures> = {\n`
  for (const key of unstableFeatureKeys) {
    output += `  ${key}: false,\n`
  }
  output += `}\n`
  ...
  console.log(
    `  ${unstableFeatureKeys.length} unstable feature(s): ${unstableFeatureKeys.join(', ')}`,
  )

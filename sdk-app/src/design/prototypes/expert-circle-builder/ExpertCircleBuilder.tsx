...
import type { Tier, Category, Expert, CartItem } from './types'
import {
  EXPERTS,
  PRE_BUILT_BUNDLES,
  CATEGORY_LABELS,
...
export function ExpertCircleBuilder() {
  ...
  const [gusMessages, setGusMessages] = useState<Array<{ role: 'user' | 'assistant'; content: string }>>([
    { role: 'assistant', content: 'Let\'s build your team of advisors! Tell me what you are looking for and I\'ll give you the options.' }
  ])
  ...
  const selectedAdvisor = selectedAdvisorChat ? EXPERTS.find(e => e.id === selectedAdvisorChat) : null
  ...
  const handleAddBundle = (bundleId: string) => {
    ...
    if (bundle) {
      const newItems = bundle.experts.filter(e => !cart.some(c => c.id === e.id)).map(e => ({ ...e, addedAt: Date.now() }))
    ...
    <div className={styles.container}>
          ...
          {showTierDropdown && (
            <div className={styles.tierDropdown}>
              {(['simple', 'plus', 'premium'] as Tier[]).map(t => (
                ...
                >
                  <span className={styles.tierLabel}>{t.charAt(0).toUpperCase() + t.slice(1)}</span>
        ...
        <div className={styles.content}>
          ...
          <div className={styles.banner}>
            <div>
              ...
              <p>Tax and benefits are already included in your plan. When you're ready for more hands-on help, on demand specialized advisory services scale with you.</p>
            </div>
          </div>
          ...
          {tier === 'plus' && (
            <div className={styles.gusCTASection}>
              <button className={styles.gusCTAButton} onClick={() => setShowGusChat(true)}>
          ...
          {/* Pre-built Bundles Section - Hidden on Simple tier */}
          {availableBundles.length > 0 && tier !== 'simple' && (
            <div className={styles.bundlesSection}>
              <h2>Recommended for you</h2>
              <p className={styles.bundleNote}>*Note: These will be event-based recommendations. For example, adding employees is a good trigger to prompt a meeting with a benefits advisor.</p>
                ...
                {availableBundles.map(bundle => (
                  <div key={bundle.id} className={styles.bundleCard}>
                    ...
                    <div className={styles.bundleExperts}>
                      {bundle.experts.map(e => (
                        <div key={e.id} className={styles.expertBadge}>
                          ...
                          {e.id === 'gusto-hrx' && (
                            <div className={styles.expertBadgePrice}>$99 per employee per month</div>
                          )}
                    ...
                    <div className={styles.bundleFooter}>
                      <div className={styles.bundlePrice}>{bundle.customPriceLabel || `$${bundle.totalMonthlyFee}/mo`}</div>
                      {isBundleFullySaved(bundle.id) ? (
                        <button className={styles.removeButton} onClick={() => handleRemoveBundle(bundle.id)}>
                          Remove
                        </button>
                      ) : (
                        <button className={styles.primaryButton} onClick={() => handleAddBundle(bundle.id)}>
                          Add All
                        </button>
                      )}
                    </div>
          ...
          <div className={styles.advisorsSection}>
            ...
            {tier === 'simple' && (
              <p className={styles.simplePlanNote}>Your plan includes free access to these advisors and they are already part of your team, reach out when you need them.</p>
            )}
            <div className={styles.cardsContainer}>
              {currentTierExperts.map(expert => (
                <ExpertCard
                  ...
                  onAdd={() => handleAddToCart(expert)}
                  onRemove={() => {
                    setCart(cart.filter(c => c.id !== expert.id))
                  }}
                  onRemoveFromSaved={() => setSavedAdvisors(savedAdvisors.filter(a => a.id !== expert.id))}
                  onDismiss={() => handleDismissCard(expert.id)}
                  onScheduleCall={(expertId) => setShowCallScheduler(expertId)}
                  onChatClick={(expertId) => {
                    setSelectedAdvisorChat(expertId)
                    setAdvisorMessages([
                      { role: 'assistant', content: `Hi! I'm ${EXPERTS.find(e => e.id === expertId)?.name}. How can I help you today?` }
                    ])
                  }}
                />
              ))}
          ...
          {showGusChat ? (
            <>
              <div className={styles.chatHeader}>
                <h2>Chat with Gus</h2>
                <button className={styles.closeChat} onClick={() => setShowGusChat(false)}>×</button>
              </div>
                ...
                <input
                  ...
                  onKeyPress={(e) => {
                    if (e.key === 'Enter' && e.currentTarget.value.trim()) {
                      setGusMessages([...gusMessages, { role: 'user', content: e.currentTarget.value }])
                      e.currentTarget.value = ''
                      // Simulate Gus response
                      setTimeout(() => {
                        setGusMessages(prev => [...prev, { role: 'assistant', content: 'Based on what you shared, here are some recommendations...' }])
                      }, 500)
                    }
                  }}
                />
            ...
            <>
              <div className={styles.chatHeader}>
                ...
                <button className={styles.closeChat} onClick={() => setSelectedAdvisorChat(null)}>×</button>
              </div>
              <div className={styles.chatMessages}>
              ...
              <div className={styles.chatInput}>
                <input
                  ...
                  onKeyPress={(e) => {
                    if (e.key === 'Enter' && e.currentTarget.value.trim()) {
                      setAdvisorMessages([...advisorMessages, { role: 'user', content: e.currentTarget.value }])
                      e.currentTarget.value = ''
                      ...
                      setTimeout(() => {
                        ...
                        setAdvisorMessages(prev => [...prev, { role: 'assistant', content: randomResponse }])
                      }, 500)
                    }
                  }}
                />
                    ...
                    <div className={styles.cartItemInfo}>
                      ...
                      <div className={styles.savedAdvisorStatus}>
                        ...
                        <button
                          ...
                          onClick={() => {
                            console.log(`${item.type === 'ai_agent' ? 'Chat' : 'Email'} with ${item.name}`)
                          }}
                        >
      ...
      {showCallScheduler && (
        <CallSchedulerModal
          ...
          onClose={() => setShowCallScheduler(null)}
        />
      )}
...
function ExpertCard({
  ...
  const isDisabledOnSimple = tier === 'simple' && (expert.id === 'ai-workforce-strategy' || expert.id === 'gusto-hrx')
      ...
      <div className={styles.expertCardContent}>
        ...
        {isLocked && expert.id !== 'ai-wage-advisor' ? (
          ...
          <>
            ...
            isDisabledOnSimple ? (
              <button
                className={`${styles.addButton} ${styles.disabledGreen}`}
                disabled
              >
                + Add
              </button>
            ...
            expert.scheduleFreeConsultation && !isInCart ? (
              <div className={styles.buttonGroup}>
                <button
                  ...
                  onClick={() => onScheduleCall(expert.id)}
                >
    ...
    <div className={styles.modalOverlay}>
      <div className={styles.modal}>
        ...
        <div className={styles.timeSlots}>
          {['Tomorrow 2:00 PM', 'Tomorrow 3:30 PM', 'Thursday 10:00 AM', 'Thursday 2:00 PM'].map(time => (
            <button
              key={time}
...
function CheckoutScreen({ tier, cart, totalMonthlyFee, onCheckoutComplete, onBack }: CheckoutScreenProps) {
      ...
      <div className={styles.checkoutContent}>
        ...
        <div className={styles.checkoutSections}>
          <section className={styles.checkoutSection}>
            ...
            <div className={styles.planBadge}>{tier.charAt(0).toUpperCase() + tier.slice(1)} Plan</div>
          </section>
          ...
          <div className={styles.confirmationScreen}>
            <div className={styles.successMessage}>
              ...
              <p className={styles.successDetails}>
                A charge of <strong>${totalMonthlyFee}/month</strong> will be added to your Gusto invoice starting on your next billing date. You can manage or adjust your advisors anytime from Help &gt; Advisors.
              </p>
            </div>
              ...
              <div className={styles.selectedExperts}>
                {cart.map((expert) => (
                  <div key={`${expert.id}-${expert.addedAt}`} className={styles.selectedExpert}>
                    ...
                    {expert.monthlyFee > 0 && <div className={styles.expertPrice}>${expert.monthlyFee}/mo</div>}
                  </div>
                ))}
              </div>
            </div>

...
const FUNCS_DIR = join(ROOT, 'node_modules/@gusto/embedded-api-v-2026-06-15/src/funcs')
const OPS_DIR = join(ROOT, 'node_modules/@gusto/embedded-api-v-2026-06-15/src/models/operations')
...
function collectTransitiveApiImports(
  ...
  function followSpec(spec: string, getResolved: () => SourceFile | undefined) {
    if (spec.startsWith('@gusto/embedded-api-v-2026-06-15/react-query/')) {
      const name = spec.slice('@gusto/embedded-api-v-2026-06-15/react-query/'.length)
      if (!name.startsWith('_')) funcNames.add(name)
    } else if (spec.startsWith('@gusto/embedded-api-v-2026-06-15/funcs/')) {
      funcNames.add(spec.slice('@gusto/embedded-api-v-2026-06-15/funcs/'.length))
